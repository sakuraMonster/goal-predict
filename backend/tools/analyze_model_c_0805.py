"""对 08-05 比赛日用 Model C 预测并分析失败原因"""
import asyncio
import json
import sys
import os
...
async def main():
    ...
    async with async_session() as db:
        ...
        for day_str in dates:
            ...
            print(f"[{day_str}] 共 {len(matches)} 场比赛\n")
    ...
    for i, r in enumerate(missed):
        ...
        print(f"    --- 新架构 ---")
        print(f"    λ_market           = {d.get('lambda_market', 'N/A')}")
        print(f"    λ_fundamental      = {d.get('lambda_fundamental', 'N/A')}")
        print(f"    divergence(盘口-基本面) = {d.get('divergence', 'N/A')}")
    ...
    # ── 保存 JSON ──
    output_path = os.path.join(os.path.dirname(__file__), "model_c_analysis_0805.json")
