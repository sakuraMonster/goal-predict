"""t4 深挖：葡超 8 场逐场对比 线上 vs 重跑(无calib) vs 重跑(×0.92 calib)"""
import asyncio
import os
import sys
...
async def main():
    ...
    async with async_session() as db:
        ...
        print(f"{'比赛':<7}{'主队':<12}{'客队':<12}{'实':>3}{'线上top2':>11}{'√':>3}{'λ':>6}{'无calib':>9}{'√':>3}{'×0.92':>8}{'√':>3}")
        for pred in preds:
            m = pred.match
            ...
            try:
                ...
                if fdf.empty:
                    print(f"{m.id:<7}{m.home_team_name or '?':<12}{m.away_team_name or '?':<12}{act_c:>3}  FEATURES EMPTY")
                    continue
                lam = model_c.predict(fdf.iloc[0].to_dict(), "葡超")["expected_goals"]
            except Exception as e:
                print(f"{m.id:<7}{m.home_team_name or '?':<12}{m.away_team_name or '?':<12}  ERR: {e}")
                continue
            rt = snap_top2(lam)
            rt92 = snap_top2(lam * 0.92)
            print(f"{m.id:<7}{m.home_team_name or '?':<12}{m.away_team_name or '?':<12}{act_c:>3}"
                  f"{str(snap_online):>11}{'√' if act_c in snap_online else '×':>3}"
                  f"{lam:>6.2f}{str(rt):>9}{'√' if act_c in rt else '×':>3}"
                  f"{str(rt92):>8}{'√' if act_c in rt92 else '×':>3}")
