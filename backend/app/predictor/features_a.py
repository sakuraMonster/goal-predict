"""
Model A 专用特征工程 —— SPF/让球预测方向，独立权重参数

从 FeatureEngineer（features.py）完整派生，继承 BaseDataFetcher 的数据库查询能力。
所有硬编码数值参数均提取为类级别常量，方便 Model A 独立调参。
"""
import pandas as pd
import numpy as np
import math
...
class FeatureEngineerA(BaseDataFetcher):
    ...
    def _extract_team_strength(self, home, away, baseline: dict = None) -> dict:
        ...
        if home and home.played and home.played > 0:
            ...
            feats["home_xG"] = home.xG or self.DEFAULT_XG
            feats["home_xGA"] = home.xGA or self.DEFAULT_XG
        ...
        if away and away.played and away.played > 0:
            ...
            feats["away_xG"] = away.xG or self.DEFAULT_XG
            feats["away_xGA"] = away.xGA or self.DEFAULT_XG
    ...
    def _extract_h2h(self, h2h_list: list, home_team_id: int, away_team_id: int) -> dict:
        ...
        if home_shots:
            feats["h2h_avg_home_shots"] = _smooth_mean(home_shots, self.H2H_DEFAULT_SHOTS_HOME)
            feats["h2h_avg_away_shots"] = _smooth_mean(away_shots, self.H2H_DEFAULT_SHOTS_AWAY)
    ...
    def _normalize_by_league(self, features: dict, baseline: dict, home_stats, away_stats) -> dict:
        ...
        home_xg = features.get("home_xG", self.RANK_DEFAULT_POS) or self.NORM_DENOM_MIN
        away_xg = features.get("away_xG", self.RANK_DEFAULT_POS) or self.NORM_DENOM_MIN
        feats["home_attacking_efficiency"] = features.get("home_goals_avg", self.RANK_DEFAULT_POS) / max(home_xg, self.NORM_EFFICIENCY_DENOM)
        feats["away_attacking_efficiency"] = features.get("away_goals_avg", self.RANK_DEFAULT_POS) / max(away_xg, self.NORM_EFFICIENCY_DENOM)
