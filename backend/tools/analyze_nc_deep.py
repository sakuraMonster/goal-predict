...
from app.predictor.snap import snap_top2_norway
...
async def main():
    ...
    async with async_session() as db:
        ...
        for m in matches:
            ...
            d = result_c["detail"]
            hit = actual_total in snap_top2_norway(eg)
            all_data.append({
    ...
    print(f"挪超 总场次: {len(all_data)}, HIT: {len(hits)}, MISS: {len(misses)}, 准确率: {len(hits)/len(all_data)*100:.1f}%\n")
    ...
    for r in sorted(all_data, key=lambda x: x["date"]):
        ...
        snap = snap_top2_norway(r["expected_goals"])
        ...
        print(f"  λ={r['expected_goals']:.2f} SNAP={snap} 偏差={delta:+.1f}{rule}")
    ...
    # Hit details
    print(f"\n{'=' * 90}")
    print("【挪超 HIT 场次明细（找出与MISS的差异）】")
    print(f"{'=' * 90}")
    for r in hits:
        snap = snap_top2_norway(r["expected_goals"])
        rule = f" [规则:{r['league_rule_applied']}]" if r["league_rule_applied"] else ""
        print(f"  [{r['date']}] {r['home']} vs {r['away']}: λ={r['expected_goals']:.2f} SNAP={snap} "
              f"实际={r['actual_total']} GL={r['goal_line']:.2f} mw={r['market_weight']:.2f}{rule}")
    ...
    for i,r in enumerate(misses):
        ...
        if delta>1.5: print(f"    严重爆大球({delta:+.1f})，GL={r['goal_line']:.2f}但实际远超")
        elif delta>0.5: print(f"    轻度爆大球({delta:+.1f})")
        elif delta<-1.0: print(f"    闷小球({delta:+.1f})")
        else: print(f"    偏差较小({delta:+.1f})，SNAP边界")

        # Check if λ adjustment could have helped
        gap_to_snap = min(abs(r["actual_total"]-s) for s in snap_top2_norway(r["expected_goals"]))
        print(f"    距SNAP最近差: {gap_to_snap}球, 需修正≈{gap_to_snap:.1f}球方向")
    ...
    for r in misses:
        ...
        gap = min(abs(r["actual_total"]-s) for s in snap_top2_norway(r["expected_goals"]))
        if delta < 1.0:
            print(f"  ✅ 偏差{delta:.1f}, SNAP差{gap}: {r['home']} vs {r['away']}")
            salvageable += 1
        elif gap <= 1:
            print(f"  △ 偏差{delta:.1f}但SNAP差{gap}: {r['home']} vs {r['away']}")
        else:
            print(f"  ✗ 偏差{delta:.1f}, SNAP差{gap}: {r['home']} vs {r['away']} (实际={r['actual_total']} GL={r['goal_line']})")
