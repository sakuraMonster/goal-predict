"""t4 回归：近30天全联赛命中率对比（线上 vs 修复后重跑，含 pipeline 联赛后验）
口径: snap_top2, actual cap=4
"""
import asyncio
import os
import sys
...
OUT = os.path.join(os.path.dirname(__file__), "t4_rerun_result.txt")
...
async def main():
    ...
    async with async_session() as db:
        ...
        p("=" * 78)
        p("近30天命中率对比: 线上(存库) vs 修复后重跑(串台过滤+sm合并+葡超×0.92) | snap_top2 cap4")
        p("=" * 78)
